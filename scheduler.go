...
func NewMesosSchedulerDriver(config DriverConfig) (initializedDriver *MesosSchedulerDriver, err error) {
	...
	fmt.Println("MESOS-GO HOSTNAME",hostname,framework.Hostname)
	...
	if newMessenger == nil {
		newMessenger = func() (messenger.Messenger, error) {
			...
			fmt.Println("MESOS-GO MESSENGER",hostname,config.BindingAddress,config.BindingPort,config.PublishedAddress)
	...
	if driver.masterDetector, err = newDetector(); err != nil {
		fmt.Println("MESOS-GO detector error!")
		return
	} else if driver.messenger, err = newMessenger(); err != nil {
		fmt.Println("MESOS-GO messenger error!")
		return
	} else if err = driver.init(); err != nil {
		fmt.Println("MESOS-GO driver init error!")
		return
	} else {
